(ns fertmfg.operation
  "FertOperationActor -- one plant-operations coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (FertAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Fertilizer Plant Operations
...
(defn build
  "Compiles a FertOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `fertmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; FertAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Fertilizer Plant Operations Governor -- independent censor.
      (g/add-node :govern
