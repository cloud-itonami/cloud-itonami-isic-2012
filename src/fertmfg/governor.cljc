(ns fertmfg.governor
  "Fertilizer Plant Operations Governor -- the independent compliance
  layer that earns the FertAdvisor the right to commit. The advisor
  ...
  tries to ACTUATE (rather than merely draft-schedule) the ammonia-
  synthesis reactor or granulation line, whether a shipment proposal's
  own claimed weight would blow through the batch's own logged
  production weight, or when an act stops being a coordination
  proposal and becomes direct synthesis/granulation-line-equipment
  control, so this MUST be a separate system able to *reject* a
  proposal and fall back to HOLD.

  `:itonami.blueprint/governor` is
  `:fertilizer-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       hallucinated `:reactor/actuate`
                                       or `:line/run`) is the 'direct
                                       synthesis/granulation-line-
    ...
    4. Line-actuate blocked        -- for `:schedule-maintenance`, does
                                       ...
                                       Directly actuating the ammonia-
                                       synthesis reactor or granulation
    ...
    8. Shipment weight exceeded    -- for `:coordinate-shipment`,
                                       ...
                                       the batch's own recorded
                                       `:weight-kg`
                                       (`fertmfg.registry/shipment-
                                       weight-exceeded?`) -- ground
                                       truth from the batch's own
                                       permanent fields, never a
                                       self-reported weight claim.
    9. Invalid product-grade       -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:product-grade` outside the
                                       closed known set
                                       (`fertmfg.registry/product-
                                       grade-valid?`), the batch record
                                       is rejected rather than let a
                                       fabricated product grade
                                       through.
   10. Invalid nutrient-content
       (N-P-K) label                -- for `:log-production-batch`, if
                                       the patch declares
                                       `:n-percent`/`:p2o5-percent`/
                                       `:k2o-percent` values that are
                                       not a physically plausible N-P-K
                                       label
                                       (`fertmfg.registry/nutrient-
                                       content-valid?`), the batch
                                       record is rejected rather than
                                       let a fabricated or mislabeled
                                       nutrient-content claim through.
   11. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [fertmfg.registry :as registry]
...
(def allowed-proposal-effects
  ...
  -- all four are propose-shaped drafts, NEVER a direct synthesis-
  reactor/granulation-line-equipment-control effect."
...
(defn- line-control-blocked-violations
  ...
  Anything else (direct ammonia-synthesis-reactor/granulation-line
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :line-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") はアンモニア合成反応器・造粒設備等の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- line-actuate-blocked-violations
  ...
  directly actuate the ammonia-synthesis reactor or granulation line --
  ...
  (when (and (= op :schedule-maintenance)
    ...
    [{:rule :line-actuate-blocked
      :detail "アンモニア合成/造粒ラインの直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- invalid-product-grade-violations
  "For `:log-production-batch`, if the patch declares a
  `:product-grade` outside the closed known set, reject rather than
  let a fabricated product grade through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [product-grade (:product-grade (:value proposal))]
      (when (and (some? product-grade) (not (registry/product-grade-valid? product-grade)))
        [{:rule :invalid-product-grade
          :detail (str product-grade " は既知の product-grade 値ではない")}]))))

(defn- invalid-nutrient-content-violations
  "For `:log-production-batch`, if the patch declares N/P2O5/K2O
  nutrient-content percentages that are not a physically plausible
  N-P-K label, reject rather than let fabricated/mislabeled data
  through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [{:keys [n-percent p2o5-percent k2o-percent] :as v} (:value proposal)]
      (when (and (or (some? n-percent) (some? p2o5-percent) (some? k2o-percent))
                 (not (registry/nutrient-content-valid? v)))
        [{:rule :invalid-nutrient-content
          :detail (str "N=" n-percent " P2O5=" p2o5-percent " K2O=" k2o-percent
                       " は物理的に妥当な N-P-K 含有率表示の範囲外")}]))))

(defn check
  "Censors a FertAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-product-grade-violations request proposal)
                           (invalid-nutrient-content-violations request proposal)))
