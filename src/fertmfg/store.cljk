(ns fertmfg.store
  "SSoT for the fertilizer-and-nitrogen-compound plant-operations
  ...
  Scope note: like its closest upstream-chemical-plant sibling
  ...
  default for dev/tests/demo, no deps. This vertical is self-contained
  (no external fertilizer-manufacturing capability library, no
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-grade/weight/N-P-K
                             nutrient-content record. `:verified?`
    ...
    - `equipment`         -- an ammonia-synthesis-reactor/granulation-
    ...
    - `maintenance`       -- a scheduled ammonia-synthesis/granulation-
                             line maintenance-window DRAFT against a
                             piece of equipment (`fertmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound fertilizer/nitrogen-
                             compound shipment DRAFT (`fertmfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (fertmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (fertmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-grade :urea :output-form :prill
                :material "Prilled Urea 46-0-0"
                :n-percent 46.0 :p2o5-percent 0.0 :k2o-percent 0.0
                :weight-kg 50000.0
                :verified? true :registered? true
                :shipped-weight-kg 10000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-grade :npk-compound :output-form :granule
                :material "NPK 15-15-15 Compound Granules"
                :n-percent 15.0 :p2o5-percent 15.0 :k2o-percent 15.0
                :weight-kg 8000.0
                :verified? true :registered? true
                :shipped-weight-kg 7500.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-grade :ammonium-nitrate :output-form :prill
                :material "Prilled Ammonium Nitrate 34-0-0"
                :n-percent 34.0 :p2o5-percent 0.0 :k2o-percent 0.0
                :weight-kg 60000.0
...
(defn- sample-equipment []
  {"synth-001" {:id "synth-001" :kind :ammonia-synthesis-reactor
                :verified? true :registered? true
                :last-maintenance-date "2026-05-01"}
   "granulator-002" {:id "granulator-002" :kind :granulation-drum
                      :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `fertmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `fertmfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  set -- one verified+registered urea batch with shipping headroom
  (schedulable), one verified+registered NPK-compound batch that is
  nearly fully shipped (a small new shipment blows through its own
  logged weight -- HARD hold), one UNVERIFIED/unregistered ammonium-
  nitrate batch (blocks any shipment coordinated against it); one
  verified+registered ammonia-synthesis-reactor unit (schedulable for
  maintenance), one UNVERIFIED/unregistered granulation-drum unit
  (blocks any maintenance scheduling against it) -- so the actor +
