(ns fertmfg.advisor
  "FertAdvisor -- the *contained intelligence node* for the fertilizer-
  and-nitrogen-compound plant-operations coordination actor.

  It normalizes production-batch patches (product-grade/weight/N-P-K
  nutrient-content), drafts an ammonia-synthesis/granulation-line
  maintenance scheduling proposal against a piece of equipment, drafts
  a safety-concern flag, and drafts an outbound fertilizer/nitrogen-
  compound shipment coordination proposal against a production batch.
  CRITICAL: it is a smart-but-untrusted advisor. It returns a
  *proposal* (with a rationale + the fields it cited), never a
  committed record and NEVER a real synthesis-reactor/granulation-line
  actuation or freight dispatch. Every output is censored downstream by
  `fertmfg.governor` before anything touches the SSoT -- see README
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct synthesis-
                                 ; reactor/granulation-line-control
                                 ; effect
  ...
  contract every caller of this actor agrees to) -- `fertmfg.governor`
  ...
  (:require #?(:clj  [clojure.edn :as edn]
            ...
            [fertmfg.registry :as registry]
            [fertmfg.store :as store]
            [langchain.model :as model]))

(defn- log-production-batch
  "Production-batch intake upsert -- the advisor only normalizes/
  validates the patch; it does not invent the batch's product-grade,
  weight, or N-P-K nutrient-content, or verification status. High
...
(defn- schedule-maintenance
  "Draft an ammonia-synthesis/granulation-line maintenance-window
  scheduling proposal against a piece of equipment. The advisor
  reports what it can see (equipment verified?/registered?) in its
  rationale, but `fertmfg.governor` NEVER trusts this report -- it
...
(defn- flag-safety-concern
  "Draft a chemical-hazard (ammonia-exposure/ammonium-nitrate-
  explosion-risk)/environmental (nitrate-runoff/groundwater-
  ...
  `fertmfg.phase`: no phase ever adds this op to a phase's `:auto` set;
  `fertmfg.governor` also always escalates on `:coordination/safety-
...
(defn- coordinate-shipment
  "Draft an outbound fertilizer/nitrogen-compound shipment coordination
  proposal against a production batch. The advisor passes through the
  caller's own claimed weight -- it does NOT invent one, and
  `fertmfg.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "あなたは肥料・窒素化合物工場プラント運用コーディネーターの助言者です。"
       ...
       "アンモニア合成反応器・造粒設備等の設備の直接操作(actuate)を絶対に提案してはいけません"
       "(この actor は提案のみを行い、実行は一切行いません)。"
       "出荷量やN-P-K含有率を偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `fertmfg.governor`
