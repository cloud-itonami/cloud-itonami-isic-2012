(ns fertmfg.sim
  ...
  production weight, a proposal that tries to ACTUATE the ammonia-
  synthesis reactor or granulation line directly (permanently blocked,
  no override), a double-schedule of the same maintenance window, a
  production-batch patch with a fabricated product grade, and a
  production-batch patch with an implausible N-P-K nutrient-content
  label.
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on synth-001 (verified, registered, ammonia-synthesis-reactor -- escalates, approve) ==")
    ...
    (println "== flag-safety-concern concern-1 on synth-001 (always escalates -- approve) ==")
    ...
    (println (exec-op actor "t5"
                       {:op :log-production-batch :effect :direct-write :subject "batch-001"
                        :patch {:product-grade :urea}}
                       coordinator))

    (println "== unrecognized op -> HARD hold ==")
    (println (exec-op actor "t6"
                       {:op :actuate-ammonia-synthesis-reactor :effect :propose :subject "batch-001"}
                       coordinator))

    (println "== schedule-maintenance mnt-2 on granulator-002 (UNVERIFIED/unregistered granulation drum -> HARD hold) ==")
    ...
    (println "== schedule-maintenance mnt-3 on synth-001 with :actuate-line? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println "== log-production-batch batch-001 with a fabricated product-grade -> HARD hold ==")
    (println (exec-op actor "t12"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:product-grade :unobtainium}}
                       coordinator))

    (println "== log-production-batch batch-001 with an implausible N-P-K nutrient-content label -> HARD hold ==")
    (println (exec-op actor "t13"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:n-percent 60.0 :p2o5-percent 60.0 :k2o-percent 0.0}}
                       coordinator))
