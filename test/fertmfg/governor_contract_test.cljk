(ns fertmfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control the ammonia-synthesis reactor or
  granulation line directly... does NOT authorize or execute
  maintenance/line operations') implemented faithfully. The single
  invariant under test:

    FertAdvisor never schedules maintenance, flags a safety concern, or
    coordinates a shipment the Fertilizer Plant Operations Governor
    would reject; `:schedule-maintenance`/`:flag-safety-concern`/
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-grade :urea}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :urea (:product-grade (store/batch db "batch-001"))) "SSoT actually updated")
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t2"
                    {:op :schedule-maintenance :effect :propose :subject "mnt-1"
                     :value {:equipment-id "synth-001" :maintenance-type :catalyst-bed-inspection
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-ammonia-synthesis-reactor :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t5"
                    {:op :schedule-maintenance :effect :propose :subject "mnt-2"
                     :value {:equipment-id "granulator-002" :maintenance-type :die-inspection
                             :scheduled-date "2026-08-01" :actuate-line? false}}
                    coordinator)]
      (is (= :hold (get-in res [:state :disposition])))
      (is (not= :interrupted (:status res)))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t8"
                    {:op :schedule-maintenance :effect :propose :subject "mnt-3"
                     :value {:equipment-id "synth-001" :maintenance-type :force-run
...
(deftest invalid-product-grade-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:product-grade :unobtainium}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-product-grade} (-> (store/ledger db) last :basis)))
    (is (not= :unobtainium (:product-grade (store/batch db "batch-001"))) "fabricated product-grade never lands in the SSoT")))

(deftest invalid-nutrient-content-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t11" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:n-percent 60.0 :p2o5-percent 60.0 :k2o-percent 0.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-nutrient-content} (-> (store/ledger db) last :basis)))
    (is (not= 60.0 (:p2o5-percent (store/batch db "batch-001"))) "fabricated N-P-K label never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "synth-001" :severity :moderate
                                            :description "ammonia odor near synthesis reactor"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "synth-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
  ...
  (testing "write-only-through-ledger: N settled operations -> N ledger facts"
    (let [[db actor] (fresh)]
      ...
      (exec-op actor "b" {:op :log-production-batch :effect :propose :subject "batch-001"
                          :patch {:product-grade :fabricated-grade}} coordinator)
