(ns fertmfg.operation-test
  "Smoke tests for the compiled FertOperationActor graph itself (build
  + one happy path per op). The governor's full rule contract (HARD
  holds, escalation, phase gating) is exercised in
  `fertmfg.governor-contract-test`; the Store contract in
  `fertmfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "FertOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "synth-001" :severity :moderate :description "ammonia odor near synthesis reactor"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
