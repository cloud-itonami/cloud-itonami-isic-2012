(ns fertmfg.registry-test
  (:require [clojure.test :refer [deftest is]]
            [fertmfg.registry :as r]))
...
(deftest known-product-grades-are-valid
  (doseq [g [:urea :ammonium-nitrate :ammonium-sulfate :calcium-ammonium-nitrate
             :uan-solution :anhydrous-ammonia :dap :map :tsp :ssp :mop :npk-compound]]
    (is (r/product-grade-valid? g))))

(deftest fabricated-product-grade-is-invalid
  (is (not (r/product-grade-valid? :unobtainium)))
  (is (not (r/product-grade-valid? nil))))

;; ----------------------------- nutrient-content-valid? -----------------------------

(deftest straight-nitrogen-label-with-only-n-percent-is-valid
  (is (r/nutrient-content-valid? {:n-percent 46.0}))
  (is (r/nutrient-content-valid? {:n-percent 34.0 :p2o5-percent 0.0 :k2o-percent 0.0})))

(deftest compound-npk-label-within-100-total-is-valid
  (is (r/nutrient-content-valid? {:n-percent 15.0 :p2o5-percent 15.0 :k2o-percent 15.0}))
  (is (r/nutrient-content-valid? {:n-percent 20.0 :p2o5-percent 10.0 :k2o-percent 10.0})))

(deftest label-with-total-exactly-100-is-valid
  (is (r/nutrient-content-valid? {:n-percent 50.0 :p2o5-percent 30.0 :k2o-percent 20.0})))

(deftest missing-nutrient-content-fields-default-to-zero-and-are-valid
  (is (r/nutrient-content-valid? {})))

(deftest negative-nutrient-value-is-invalid
  (is (not (r/nutrient-content-valid? {:n-percent -1.0})))
  (is (not (r/nutrient-content-valid? {:n-percent 10.0 :p2o5-percent -5.0}))))

(deftest single-field-over-100-is-invalid
  (is (not (r/nutrient-content-valid? {:n-percent 150.0}))))

(deftest total-over-100-is-invalid-even-when-each-field-individually-in-range
  (is (not (r/nutrient-content-valid? {:n-percent 60.0 :p2o5-percent 60.0 :k2o-percent 0.0}))
      "60 + 60 = 120% of the product's own mass -- physically impossible"))

(deftest non-numeric-nutrient-value-is-invalid
  (is (not (r/nutrient-content-valid? {:n-percent "46"}))))

;; ----------------------------- register-maintenance -----------------------------

(deftest maintenance-is-a-draft-not-a-real-actuation
  (let [result (r/register-maintenance "mnt-1" "synth-001" 0)]
    (is (nil? (get-in result ["certificate" "proof"])))
    (is (= (get-in result ["certificate" "issued_by_registry"]) false))
    (is (= (get-in result ["certificate" "status"]) "draft-unsigned"))))

(deftest maintenance-assigns-maintenance-number
  (let [result (r/register-maintenance "mnt-1" "synth-001" 7)]
    (is (= (get result "maintenance_number") "MNT-000007"))
    (is (= (get-in result ["record" "maintenance_id"]) "mnt-1"))
    (is (= (get-in result ["record" "equipment_id"]) "synth-001"))
    (is (= (get-in result ["record" "kind"]) "maintenance-schedule-draft"))
    (is (= (get-in result ["record" "immutable"]) true))))

(deftest maintenance-validation-rules
  (is (thrown? #?(:clj Exception :cljs js/Error) (r/register-maintenance "" "synth-001" 0)))
  (is (thrown? #?(:clj Exception :cljs js/Error) (r/register-maintenance "mnt-1" "" 0)))
  (is (thrown? #?(:clj Exception :cljs js/Error) (r/register-maintenance "mnt-1" "synth-001" -1))))
...
(deftest history-is-append-only
  (let [c1 (r/register-maintenance "mnt-1" "synth-001" 0)
        hist (r/append [] c1)
        c2 (r/register-maintenance "mnt-2" "synth-001" 1)
